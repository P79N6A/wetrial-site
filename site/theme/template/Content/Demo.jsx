...
function antdProCodeFormat(c) {
  let code = c;
  code = code.replace(
    /import\s+\{\s+(.*)\s+\}\s+from\s+'wetrial\/lib\/(.*)';/gi,
    'import { $2 } from "wetrial";\nconst { $1 } = $2;'
  );
  code = code.replace(
    /import\s+(.*)\s+from\s+'wetrial\/lib\/(.*)';/gi,
    'import { $1 } from "wetrial";'
  );
  return code;
}
...
function antdProCodeFormatCodepen(c) {
  ...
  code = code.replace(/import\s+(.*)\s+from\s+'wetrial\/lib\/(.*)';/gi, '');
  return code;
}
  ...
  render() {
    ...
    const dependencies = state.sourceCode.split('\n').reduce(
      (acc, line) => {
        ...
        if (matches && matches[1] && !line.includes('wetrial/lib/')) {
    ...
    const codepenPrefillConfig = {
      ...
      editors: '001',
      css_external:
        'https://unpkg.com/antd@next/dist/antd.css;https://unpkg.com/wetrial/dist/wetrial.css',
      js_external: [
...
import './index.css';
import 'wetrial/dist/wetrial.css';
